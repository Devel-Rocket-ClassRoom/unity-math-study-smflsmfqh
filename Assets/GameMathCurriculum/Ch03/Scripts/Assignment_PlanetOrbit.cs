...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private float satelliteAngle = 0f;

    private void Update()
    {
        // TODO, 행렬 변환해서 직접 포지션을 할당
        // 자식 오브젝트가 붙어있지 않은데 마치 붙은 것처럼 변환행렬 이용해서
        // 가상의 부모가 있다고 생각, 가상 부모의 trs 행렬 만들고, 위성의 로컬 오프셋 계산해서 행렬 곱으로 월드 좌표 구하기
        // trs 두 번 하는 거, 행성, 위성

        planetAngle = planetOrbitSpeed * Time.time;
        satelliteAngle = satelliteOrbitSpeed * Time.time;

        Vector3 planetLocalOffset = new Vector3(planetOrbitRadius, 1f, 0f);
        ...
        planetWorldPos = planetMatrix.MultiplyPoint(planetLocalOffset);
        transform.position = planetWorldPos;

        Vector3 satelliteLocalOffset = new Vector3(satelliteOrbitRadius, 0f, 0f);   
        ...
        satelliteWorldPos = satelliteMatrix.MultiplyPoint(satelliteLocalOffset);   
