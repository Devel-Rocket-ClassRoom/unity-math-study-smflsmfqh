...
public class Assignment_SplineConveyor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float spacing = 1f / (boxes.Length + 1);

        for (int i = 0; i < boxes.Length; i++)
        {
            float offsetT = globalT + i * spacing;
            offsetT = Mathf.Repeat(offsetT, 1f);

            boxes[i].position = EvaluateSpline(waypoints, offsetT);
        }
    ...
    private Vector3 EvaluateSpline(Transform[] pts, float t)
    {
        ...
        float scaledT = t * segmentCount; // 매개변수로 받은 t로 구간 쪼갬
        int segment = Mathf.Clamp((int)scaledT, 0, segmentCount - 1); 
        float localT = scaledT - segment; // 한 구간의 범위인 scaledT의 진행률을 나타낼 t = localT
