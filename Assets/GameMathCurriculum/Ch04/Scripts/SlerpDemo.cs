...
public class SlerpDemo : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Quaternion newTargetRotation = Quaternion.LookRotation(target.position - transform.position); // 이 방향을 바라보는 회전 계산
    ...
    private void UpdateAbsoluteSlerp()
    {
        ...
        interpolationDuration = 1f / rotationSpeed; // rotationSpeed가 각속도 관점이므로, 1초에 rotationSpeed 만큼 회전하도록 설정
