...
public class LerpBasics : MonoBehaviour
{
    ...
    private void UpdateHPLerp()
    {
        if (useAbsoluteLerp)
        {
            ...
            float t = Mathf.Clamp01(lerpElapsedTime / absoluteLerpDuration); // 0 ~ 1 사이로 정규화
            displayHP = Mathf.Lerp(lerpStartHP, lerpTargetHP, t);

            if (t >= 1f) // 최댓값에 도달할 때 = 목표 HP에 도달했을 때 종료 조건 - 패턴
            {
                lerpStartHP = displayHP = lerpTargetHP; // 정확히 목표 HP로 설정
                lerpElapsedTime = 0f;
            }
        }
        else
        {
            displayHP = Mathf.Lerp(displayHP, currentHP, hpLerpSpeed * Time.deltaTime);
            if (Mathf.Abs(displayHP - currentHP) < 0.01f) // 거의 도달했을 때 종료 조건 - 패턴
            {
                displayHP = currentHP; // 정확히 현재 HP로 설정
            }
        }
    }
    ...
    {
        colorLerpTime = (colorLerpTime + Time.deltaTime) % (2f / colorLerpSpeed); // 0.0 ~ 2.0 사이의 시간 루프(1초마다 완전한 사이클)
        float colorT = Mathf.PingPong(colorLerpTime * colorLerpSpeed, 1f); // 0 ~ 2까지의 값을 받되 리턴해주는 float값이 0 -> 1 -> 0 사이로 왔다갔다 하도록 만들어주는 함수
