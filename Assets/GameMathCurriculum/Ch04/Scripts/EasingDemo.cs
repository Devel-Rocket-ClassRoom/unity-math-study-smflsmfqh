...
public class EasingDemo : MonoBehaviour
{
    ...
    private void UpdateJump()
    {
        ...
        // 0 -> 1 -> 0 형태로 점프 궤적 만들기 위해 (t * (1-t)) 형태로 곡선 조정
        float jumpCurve = currentEasedValue * (1f - currentEasedValue) * 4f; // 0~1 사이에서 최대 1이 되도록 조정

        Vector3 newPos = startPos;
        newPos.y = startPos.y + jumpCurve * jumpHeight;
        transform.position = newPos;
        if (currentT >= 1f)
        {
            isJumping = false;
            transform.position = startPos; // 정확히 원래 위치로 복원
        }   
    }
    ...
    private void UpdateMove()
    {
        ...
        Vector3 newPos = startPos;
        newPos.x = startPos.x + currentEasedValue * moveDistance;
        transform.position = newPos;
